...
with st.expander("사용법 펼치기"):
    ...
    2.  엑셀 파일을 업로드하세요. (.xlsx)
    - 업로드 전 반드시 문제 번호에 대하여 오름차순 정렬을 시행한 후 업로드 해주세요.
    ...
    4. 파일명을 입력하세요.       
    - 저장되는 Word 파일명입니다. 확장자는 자동으로 추가됩니다.
    - 기본값: 제목 없음
                
    5. Convert 버튼을 누르면 변환이 시작됩니다.
    - 실행 시간은 100문제 기준 1-3분 소요됩니다.

    6. Download 버튼을 누르면 Word 파일이 다운로드됩니다.          
                
    7. 유의사항
    - API Key는 절대로 외부에 공유하지 말아주세요.
    - 문제 번호 항목이 정수가 아닐 경우 "?"로 표시됩니다.
    - 문제 내용 항목이 비어있는 경우 "복원 실패"로 표시됩니다.  
    - 문제 내용은 GPT-4o를 통해 자동 분석되므로 API 사용량에 유의하세요.
    """)
...
def process_question(number, q_text, client, doc):
    prompt = f"""
문제와 선지를 분리해주세요. 문제는 여러문장 일 수 있고 가나다라 보기도 문제에 포함해주세요. 선지는 '① 선택지내용' 형식으로 출력해주세요. 선지는 최대한 원문 그대로 보존하되 오타나 맞춤법 이상이 있다면 틀린 부분만 교정해주세요.
